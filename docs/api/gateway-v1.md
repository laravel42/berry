...
This document is the HTTP and Server-Sent Events (SSE) reference for Berry's product API, served by `server-ts/`. It defines Berry-owned resource names and shapes; runtime-provider payloads are adapted at the API boundary and are never relayed directly to clients.
...
This document tracks the routes actually mounted by `server-ts/src/index.ts`. `server-ts/SCOPE.md` lists the prefixes that are intentionally unserved (404 by design, not a bug); `server-ts/ROUTING.md` groups the mounted prefixes by area. Where the database and this contract differ, this contract wins; storage names are not API field names. Sections below marked "route table only" describe an area at a high level — for full request/response shapes, read the mount source under `server-ts/src/mounts/`.

Shipped but not yet given a full schema here — treat the implementation as authoritative:
...
- **Integrations** — `GET /api/v1/integrations/providers` (catalogue joined with this workspace's state), `GET`/`DELETE /api/v1/integrations/connections`, `POST /api/v1/integrations/connections/{provider}/authorize`, `GET /api/v1/integrations/github/repositories`, `GET /api/v1/integrations/github/repository-owners` (`{ owners: [{ login, type }] }`: the signed-in person's account and the organisations that let them create repositories, read with their own token), and `POST /api/v1/integrations/github/repositories` (`{ name, owner?, private? }`; creates an empty repository with the signed-in person's own GitHub token, under their account or an organisation they may create in, and returns `{ repository }` for the project dialog to link; 409 `REPOSITORY_EXISTS` when the name is taken, 403 when GitHub will not let them create there, 409 `SIGN_IN_REQUIRED` without a GitHub sign-in). A provider whose stored credential has passed `expiresAt` reports `status: "expired"` and `connected: false`, derived rather than read from a column, so this endpoint and `/connections` cannot disagree.
...
Agents support full configuration: an authorized caller can create (`POST /api/v1/agents`, `Idempotency-Key`), copy, restore, and set an agent's config, permissions, role contract, labels, env and avatar (route table below).
...
#### Other agent routes (route table only)

| Route | Purpose |
|---|---|
| `POST /api/v1/agents` | Create an agent (`Idempotency-Key`) |
| `POST /api/v1/agents/{agentId}/copy` | Copy an agent (`Idempotency-Key`) |
| `POST /api/v1/agents/{agentId}/restore` | Restore an archived agent |
| `GET /api/v1/agents/roster` | Full roster including organization role agents (not returned by the paginated `GET /agents`) |
| `GET /api/v1/agents/guide` | The workspace's Guide agent (`system_role: "guide"`) |
| `GET /api/v1/agents/models` | Runtime model catalog |
| `PUT /api/v1/agents/{agentId}/permissions` | Repository and tool permission overrides |
| `PUT /api/v1/agents/{agentId}/contract` | Organization role contract (mission, autonomy level, allowed tools, delegation/escalation rules) — see [Organization](#organization) |
| `PUT /api/v1/agents/{agentId}/labels` | Label routing rules |
| `PUT /api/v1/agents/{agentId}/env` | Sealed runtime-profile environment variables |
| `PUT /api/v1/agents/{agentId}/avatar` | Avatar image |
...
| Route | Purpose |
|---|---|
| `GET /api/v1/organization` | Roles grouped by department, with discovery state |
| `PUT /api/v1/organization/discovery` | Enable/disable weekly autonomous discovery for the workspace |
| `POST /api/v1/organization/roles/{roleKey}/reset` | Restore a role's contract to the shipped catalog version |
| `PUT /api/v1/agents/{agentId}/contract` | Edit one agent's role contract directly (see [Agents](#agents)) |

No autonomy level grants a tool that sets an issue to `done` or `cancelled`, and none includes a merge tool — release is always a person's decision, and a reviewer agent's approval is advisory or blocking, never a release itself.

### Work proposals (route table only)

Agents can propose new work through the `propose_work` tool rather than creating issues directly. A proposal is an issue labelled `proposal`, deduplicated on a fingerprint; it is auto-accepted into `todo` (`auto_accept`) only for a Level 4+ proposer with non-critical/high severity and routine impact; otherwise it is parked in `backlog` behind a `work_proposal` approval (`needs_decision`).

| Route | Purpose |
|---|---|
| `GET /api/v1/work-proposals` | List proposals; filters `status`, `role`, `severity` |

### Runtimes (route table only)

An `agent_runtimes` row describes one execution target: `platform` (the server's default, synced from its own configured target) or `custom`, over the `agentcore` or `http` driver. Runtime profiles attach environment, model default, timeout, and concurrency settings, and can be bound to specific agents.

| Route | Purpose |
|---|---|
| `GET /api/v1/runtimes`, `POST /api/v1/runtimes` | List / register a runtime |
| `GET /api/v1/runtimes/agent-coverage` | Which agents have no runtime binding |
| `GET`, `PATCH`, `DELETE /api/v1/runtimes/{id}` | Read, update, remove a runtime |
| `POST /api/v1/runtimes/{id}/health` | Probe a runtime |
...
This prefix is how a running agent acts on Berry — the only path it has, since agents never touch the database directly. It accepts a short-lived, task-scoped bearer token only (minted per run, TTL at most 8 hours); every other credential type is refused here, and a task token is refused everywhere else.

| Route | Purpose |
|---|---|
| `GET /api/v1/agent-tools` | List tools available to the calling agent for its current task |
| `POST /api/v1/agent-tools/{name}` | Call one tool |

Available tools are the intersection of the agent's role contract (`allowed_tools`) and its autonomy-level ceiling. Core tools include `read_task`, `list_files`, `read_file`, `write_file`, `set_status` (only `todo | in_progress | in_review | blocked` — never `done` or `cancelled`), `run_command` and `mention_agent`; organization tools add `escalate`, `delegate_to_agent`, `propose_work` and `submit_review` (autonomy level 5 only). Calling a tool that does not exist or is outside the task's scopes answers `404 NOT_FOUND`. Inside the runtime, a permissions hook in front of every tool refuses unknown tools before any HTTP call and returns the refusal to the model as the tool result.
...
1. Agent creation, copy, restore and editing are served (see [Agents](#agents)).
