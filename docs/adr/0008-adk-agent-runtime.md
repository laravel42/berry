...
- **Status:** Superseded by [ADR-0013](0013-strands-native-agent-runtime.md)
  (Strands, natively) and then [ADR-0014](0014-agentcore-runtime-control-plane.md)
  (the loop moves out of Berry's process into AgentCore Runtime). The
  decision this record stands for — agents run as configuration Berry owns,
  not as a separate service Berry discovers — still holds; the library (ADK)
  and the in-process mechanism do not. The runtime this record replaces was
...
- Superseded twice over: ADK was replaced by the Strands Agents SDK
  (ADR-0013), and the in-process loop itself was then moved into AgentCore
  Runtime (ADR-0014). Nothing in this ADR's mechanism runs today.
- What survives from this decision: Berry authors and owns agents as
  configuration (no separate runtime service to discover/sync), artifacts are
  written directly rather than swept from a volume (ADR-0006's 2026-08-28
  amendment), and tools are constructed with the calling workspace in scope.
- Current implementation lives in `server-ts/src/agents/runtime/` (the
  Strands loop and plugins) and `server-ts/sandbox/agentcore/` (the runtime
  image), per ADR-0014.
- Temporal, named throughout this ADR as staying for durable orchestration,
  is also gone from the current design; the dispatcher (`src/runs/
  dispatcher.ts`) claims work with `SKIP LOCKED` under a renewed lease
  instead.
