...
- **Status:** Partially implemented — Runtime delivered (since
  [ADR-0014](0014-agentcore-runtime-control-plane.md), where the loop runs),
  Memory delivered (optional), Gateway implemented but dormant, Policy not
  started.
- **Date:** 2026-09-09
...
- **Runtime:** delivered, and superseded in shape by ADR-0014 — the Strands
  loop itself now runs inside the AgentCore Runtime image
  (`server-ts/sandbox/agentcore/`), not merely invoked one shell command at a
  time from Berry's process as this ADR originally scoped. Selected when
  `BERRY_AGENTCORE_RUNTIME_ARN` is set (target selection in
  `server-ts/src/index.ts`).
- **Gateway:** implemented, dormant by default. Active only when both
  `AWS_AGENTCORE_GATEWAY_URL` and `AWS_AGENTCORE_GITHUB_PROVIDER` are set;
  `GITHUB_PROVIDER` defaults to `agentcore` and selects the Gateway route for
  GitHub API calls when configured, otherwise falling back to the GitHub
  App/OAuth path.
- **Memory:** delivered for short-term run recall. `BERRY_AGENTCORE_MEMORY_ID`
  enables it; `runtime/envelope-builder.ts` wires AgentCore Memory into the
  envelope when set. Long-term semantic extraction is still not implemented.
- **Policy:** not implemented; still blocked on Gateway per this ADR's
  original sequencing.
- There is no Docker Compose stack anymore, so the `berryminio` credential
  problem described under Phase 3 evidence is historical.
