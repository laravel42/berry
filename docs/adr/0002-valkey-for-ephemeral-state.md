...
- **Status:** Accepted — decision stands; Valkey itself remains optional and
  unwired (see "Current state" below)
- **Date:** 2026-08-22
...
- Valkey is optional and unwired in every environment, not only development:
  the broadcaster is always constructed with a null relay
  (`server-ts/src/index.ts`, "a hub with no relay for now"), no Valkey/Redis
  environment variable is read anywhere in `src/`, and `capabilities.valkey`
  is hard-coded `false`.
- The decision itself stands: PostgreSQL (`outbox_events`) is the durable
  system of record, and realtime events reach other processes by polling and
  replay rather than a live relay.
- `ioredis` remains a dependency and `src/realtime/relay.ts` is real code, but
  nothing constructs it with a live connection.
- There is no Docker Compose stack anymore (removed); a future Valkey, if
  ever wired, would run as a separate host service.
