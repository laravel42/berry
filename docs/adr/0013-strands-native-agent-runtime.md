...
- **Status:** Partially superseded by
  [ADR-0014](0014-agentcore-runtime-control-plane.md): the loop no longer
  runs in Berry's own process, it runs inside AgentCore Runtime, using the
  same sources. This ADR's choice of SDK, plugins and retry strategy stands
  unchanged — only *where* the loop executes moved.
- **Date:** 2026-09-09
...
- The SDK choice and its plugin shape are live and unchanged: `LedgerPlugin`,
  `PermissionPlugin`, `AccountingPlugin`, `ToolOutcomePlugin` and
  `BerryRetryStrategy`, in `server-ts/src/agents/runtime/`.
- What changed is *where* this code runs: ADR-0014 moved it from an
  in-process loop inside the API server into the AgentCore Runtime image
  (`server-ts/sandbox/agentcore/`), built from the same sources under
  `--experimental-strip-types`. The API process no longer holds a model
  client or Bedrock credentials for the run loop.
- Single completions (planner, triage, chat reply, editor) still run as
  toolless, `structuredOutputSchema`-driven calls, now dispatched as
  `kind: 'completion'` tasks per ADR-0014 rather than calling a local
  Bedrock client directly.
- The declined items (`SessionManager`, `sandbox/docker`, `multiagent`
  Graph/Swarm, the Cedar intervention, `memoryManager`) are still declined,
  unchanged by ADR-0014.
