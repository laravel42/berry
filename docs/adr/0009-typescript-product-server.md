...
- Live and unchanged in substance: `server-ts` (`@berry/server`) is the one
  product server, run directly with `node --experimental-strip-types` and no
  build step, per `server-ts/package.json` and `server-ts/Dockerfile`.
- Temporal, described here as staying for durable orchestration, is gone:
  there is no `@temporalio/*` dependency in `server-ts/package.json`. Run
  dispatch is now `src/runs/dispatcher.ts`, claiming work with `SKIP LOCKED`
  under a renewed lease rather than a Temporal workflow.
  `server-ts/ARCHITECTURE.md` and `server-ts/SCOPE.md` describe the current
  shape; treat them as authoritative over this ADR's Temporal-era detail.
- The agent runtime named here (ADK) is gone; see ADR-0008, ADR-0013 and
  ADR-0014 for what replaced it and runs today.
- `server-ts/SCOPE.md` still records what is intentionally unserved.
