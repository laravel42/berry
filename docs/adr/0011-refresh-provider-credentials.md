...
- **Status:** Partially superseded, for GitHub, by Berry's own GitHub App
  (migration `040`, `src/integrations/github-app.ts`). Proposed for every
  other provider reached through user OAuth. Later the same day Berry gained
  its own GitHub App, which mints installation tokens on demand and so has no
  user token to refresh. This record still applies to any provider Berry
  reaches through user OAuth, and to a deployment that has not created an
  App; it is no longer the fix for GitHub.
- **Date:** 2026-09-01
...
- For GitHub, superseded: repository access uses per-run installation tokens
  minted from Berry's own GitHub App (`src/integrations/github-app.ts`), so
  there is no user token to refresh.
  `GITHUB_CLIENT_ID`/`GITHUB_CLIENT_SECRET` remain as the older, legacy
  user-token connection path, separate from sign-in.
  `GITHUB_PROVIDER` defaults to `agentcore` and can fall back to `legacy`.
- For any other OAuth-connected provider, this ADR's refresh design is still
  the proposed fix; it has not been implemented for another provider because
  none exists yet.
- Sign-in itself is unrelated: Better Auth with a dedicated GitHub OAuth App
  (`BERRY_AUTH_GITHUB_CLIENT_ID`/`_SECRET`), not the repository-access path
  this ADR is about.
