...
- **Status:** Accepted — decision stands; the storage backend has moved on
  from the mechanism described below (see "Current state")
- **Date:** 2026-08-24
...
- The core decision stands: agent-produced artifacts are ordinary
  `attachments` rows in object storage, with `uploader_type`/`uploader_id`
  and a nullable `run_id` (`server-ts/src/storage/`).
- The backend is AWS S3 by default, not MinIO: `S3_BUCKET` enables storage,
  `S3_ENDPOINT` can point at any S3-compatible store, and
  `S3_USE_PATH_STYLE` still defaults to `true` — a leftover default from the
  MinIO era rather than a sign MinIO is still used.
- There is no Docker Compose stack (removed), so the "local compose stack
  runs MinIO" development story below is historical; local development also
  talks to S3 (or an S3-compatible endpoint) directly.
- Under ADR-0014, an agent writes artifacts by calling
  `/api/v1/agent-tools/*` (for example `write_file`/`attach_file`) with a
  task-scoped token, from inside the AgentCore Runtime container — not by a
  worker reading a shared runtime volume. The "Promotion" mechanism described
  below (read-only volume mount, post-run sweep) does not exist; see the
  2026-08-28 amendment above.
