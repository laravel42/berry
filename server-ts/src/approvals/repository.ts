...
export type ApprovalKind = 'plan' | 'issueStart' | 'integrationAction' | 'workProposal' | 'escalation';
...
/** A decided approval, and the task (if any) the decision handed back to an agent to work on. */
export interface ResolvedApproval extends Approval {
   /**
    * Set for an accepted work proposal assigned to its role and for an
    * answered escalation: the caller starts that task after the commit, which
    * this repository cannot do inside its own transaction.
    */
   releasedIssueId: string | null;
}
   ...
   async resolve(input: {
      ...
      return this.#sql.begin(async (transaction) => {
         ...
         const [current] = await tx`
            SELECT id, kind, issue_id, status, risk, requested_from_user_id, requested_from_role
         ...
         if (current.kind === 'work_proposal') {
            releasedIssueId = await applyProposalDecision(tx, { approvalId: current.id as string, decision: input.decision, userId: input.userId });
         ...
         return { ...toApproval(row!), releasedIssueId };
      }) as Promise<ResolvedApproval>;
   }
}
