...
describe(
   ...
   () => {
      ...
      /** `/user/repos` and `/repos/{owner}/{name}` as GitHub answers a `repo`-scoped token. */
      function userReposStub(calls: string[] = []): typeof globalThis.fetch {
         const rows = [
            {
               id: 301,
               name: 'notes',
      ...
      test('lists the repositories the sign-in token can push to, private ones included', async () => {
         const calls: string[] = [];
         const listed = await access(userReposStub(calls)).listRepositories(u1Id);
         assert.deepEqual(
            listed.map((repository) => [repository.fullName, repository.private, repository.owner]),
            [['alice/notes', true, 'alice']]
         );
      ...
      test('resolves a repository through the sign-in token, and a 404 is null', async () => {
         const user = access(userReposStub());
         const found = await user.resolveRepository(u1Id, 'alice', 'notes');
         assert.equal(found?.id, 301);
         assert.equal(found?.fullName, 'alice/notes');
         assert.equal(await user.resolveRepository(u1Id, 'acme', 'secret'), null);
      });

      test('listing with a refused token says sign in again and forgets the token', async () => {
         await assert.rejects(
            access(githubStub([], { status: 401 })).listRepositories(u1Id),
            (error: unknown) =>
