...
test("an autonomy allowlist cancels a tool outside it, and lets an allowed one through", async () => {
   ran = 0;
   ...
   const agent = new Agent({
      model,
      tools: [runCommand, readTask],
      plugins: [
         new PermissionPlugin({
            permissions: permissionsOf(['run_commands'], 'Bot'),
            allowed: new Set(['read_task']),
         }),
      ],
      printer: false,
   });
   await agent.invoke('go');
   assert.equal(ran, 0);
   assert.match(received(model, 1), /outside this agent's autonomy level/);
});

test('a remote or MCP tool name is exempt from the autonomy allowlist', async () => {
   const model = new ScriptedModel([call('read_task', {}), say('ok')]);
   const agent = new Agent({
      model,
      tools: [readTask],
      plugins: [
         new PermissionPlugin({
            permissions: noPermissions('Bot'),
            allowed: new Set(['some_other_tool']),
            exempt: new Set(['read_task']),
         }),
      ],
      printer: false,
   });
   await agent.invoke('go');
   assert.doesNotMatch(received(model, 1), /autonomy level/);
});
