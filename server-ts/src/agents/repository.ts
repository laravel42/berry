...
export interface Agent {
   ...
   createdAt: string;
   updatedAt: string;
   /** The catalog role this agent fills, or null for an agent nobody assigned one to. */
   roleKey: string | null;
   /** The role's department, read off its contract; null without a valid contract. */
   department: string | null;
   autonomyLevel: number | null;
   /** True once the stored contract no longer hashes to what Berry last wrote — a person edited it. */
   customized: boolean;
...
/** Restoring a role agent whose role another live agent in the workspace already fills. */
export class RoleTaken extends Error {
   constructor() {
      super('another live agent already fills this role');
      this.name = 'RoleTaken';
   }
}
...
const AGENT_COLUMNS = `
   ...
   agent.role_key, agent.role_contract, agent.contract_hash, agent.contract_version, agent.autonomy_level,
...
export class AgentRepository {
   ...
   async archive(agentId: string, workspaceId: string, now: Date): Promise<void> {
      ...
      await this.sql.begin(async (transaction) => {
         ...
         await tx`
            UPDATE agents SET archived_at = ${at}, updated_at = ${at}
             WHERE id = ${agentId} AND workspace_id = ${workspaceId} AND archived_at IS NULL`;
         // Archiving a role agent removes the role: its weekly discovery stops
         // with it rather than firing for an agent that can no longer run.
         await tx`
            UPDATE autopilots AS pilot SET status = 'paused', updated_at = ${at}
              FROM agents AS agent
   ...
   async restore(agentId: string, workspaceId: string): Promise<Agent> {
      ...
      await this.sql.begin(async (transaction) => {
         ...
         const restored = await tx<Array<{ role_key: string | null }>>`
            ...
            // fills this role (a duplicate from before archived roles were
            // treated as removed).
            if ((error as { code?: string })?.code === '23505') throw new RoleTaken();
            throw error;
         });
         const roleKey = restored[0]?.role_key ?? null;
         if (!roleKey) return;

         // The role is back, and so is its discovery. Resuming starts the
         // schedule from now, as AutopilotRepository.update does, so a slot
         // that passed while the agent was archived does not fire at once.
         const resumed = await tx<Array<{ id: string }>>`
            UPDATE autopilots SET status = 'active', updated_at = now()
             WHERE workspace_id = ${workspaceId} AND discovery_role = ${roleKey}
               AND assignee_type = 'agent' AND assignee_id = ${agentId}
               AND archived_at IS NULL AND status = 'paused'
            RETURNING id`;
         const from = new Date();
         for (const pilot of resumed) {
            const triggers = await tx<Array<{ id: string; cron_expression: string; timezone: string }>>`
               SELECT id, cron_expression, timezone FROM autopilot_triggers
                WHERE autopilot_id = ${pilot.id} AND kind = 'cron' AND enabled`;
            for (const trigger of triggers) {
               const next = nextFireAfter(trigger.cron_expression, trigger.timezone, from);
               await tx`
    ...
    *
    * `contract_hash` and `contract_version` are deliberately left alone: they
    * record what Berry last wrote (the catalog's version), so a hand edit
    * here is what makes `customized` become true on the next read. `model_name`
    * is likewise untouched — a contract edit changes the role, not the runtime
    * bound to it.
    */
   async writeRoleContract(
      agentId: string,
      workspaceId: string,
      ...
      const [row] = await this.sql`
         UPDATE agents AS agent
            SET role_contract = ${this.sql.json(contract as never)},
                instructions = ${contract.system_prompt},
                description = ${contract.mission},
                capabilities = ${contract.capabilities},
                autonomy_level = ${contract.autonomy_level},
                permissions = ${permissions},
                -- The contract's run limits are what runs read, as provisioning writes them.
                manifest_limits = ${this.sql.json({
                   maxTurns: contract.run_limits.max_turns,
                   maxTokens: contract.run_limits.max_output_tokens,
                } as never)},
                updated_at = now()
          WHERE agent.id = ${agentId} AND agent.workspace_id = ${workspaceId}
            AND agent.archived_at IS NULL
