 ...
 * App is for repository work only — sign-in is a separate GitHub OAuth App
 * configured from BERRY_AUTH_GITHUB_*. Its secrets are sealed
...
/**
 * Sign-in: Better Auth's GitHub provider on a GitHub OAuth App, from
 ...
 */
const scm = await createScm({ sql, config, logger, githubApp, userAccess: githubUserAccess });
...
// Every workspace also carries the skills pack, and its agents run on the
// deployment's own runtime unless a person bound them elsewhere. After the
// organization, so the agents it just provisioned are bound too.
const deploySkills = loadDeploySkills((error) =>
   logger.error('could not read the skills pack', {
      error: error instanceof Error ? error.message : String(error),
   })
);
await seedDefaultsEverywhere(sql, deploySkills, (workspaceId, error) =>
...
const commentOptions = {
   ...
   // A person's comment that mentions an agent (or replies to the assignee)
...
registry.registerAll(
   approvalMounts({
      ...
      issues,
      idempotency,
      // An accepted proposal or an answered escalation starts its task, the
      // same auto-dispatch an assignment gets. Only where runs can execute.
      ...(workDispatch ? { dispatch: workDispatch } : {}),
      stages: stageGate(sql),
      onDispatchError: (issueId, error) =>
         logger.error('could not start a task an approval released', {
            issueId,
            error: error instanceof Error ? error.message : String(error),
         }),
   })
...
if (auth) {
   registry.registerAll(betterAuthMounts({ handler: (request) => auth.handler(request) }));
}
registry.registerAll(
   platformMounts({
...
logger.info('Berry server listening', {
   ...
   integrations: connections
      ...
      : 'no encryption key',
   // Named at boot so a missing BERRY_AUTH_GITHUB_* is visible before anyone
   // meets a disabled button.
   signIn: auth ? (config.auth.github ? 'github (OAuth App)' : 'no BERRY_AUTH_GITHUB_* credentials') : 'off',
...
// rather than failing at the socket — but only for a few seconds: event streams
// never end on their own, and a close that waited on them would never exit.
const SHUTDOWN_GRACE_MS = 5_000;
for (const signal of ['SIGINT', 'SIGTERM'] as const) {
   process.once(signal, () => {
      logger.info('shutting down', { signal });
      void drain(server, { graceMs: SHUTDOWN_GRACE_MS }).then(() => {
