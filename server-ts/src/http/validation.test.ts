...
test('a locale must be one Berry ships a catalogue for, spelled exactly', () => {
   assert.ok(validLocale('en'));
   // Case and region variants are refused rather than normalised: the stored
   // value is also the catalogue directory name the frontend loads. ja, ko and
   // zh-Hans used to ship; they are refused now that those catalogues are gone.
   for (const locale of ['EN', 'zh-Hans', 'ja', 'ko', 'zh', 'zh-hans', 'zh-CN', 'fr', '', ' en'])
