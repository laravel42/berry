...
import { EnqueueRejected } from '../runs/queue.ts';
...
describe('issue-tracking routes', { skip: url ? false : 'BERRY_TEST_DATABASE_URL is not set' }, () => {
   ...
   before(async () => {
      ...
      // Refuses the way `enqueueTask` does: a busy issue already has a run,
      // and an archived agent is no longer in the workspace.
      const enqueue: QuickActionEnqueue = async (_sql, input) => {
         if (input.issueId && busy.has(input.issueId)) throw new ActiveRunExists('run-already-going');
         if (input.prompt === 'Gone agent') {
   ...
   test('a quick action on a task with a run in progress is a 409, not a 500', async () => {
      const action = await createQuickAction(sql, world.workspaceId, world.ownerId, quickActionCreateSchema.parse({ name: 'Busy check', targetAgentId: world.agentId, prompt: 'Check {{issue.title}}' }));
      const task = await createIssue(sql, world, { title: 'Busy task' });
      busy.add(task);
      const refused = await call('POST', `/api/v1/issues/${task}/quick-actions/${action.id}/run`, 'owner');
      assert.equal(refused.status, 409);
      const error = refused.body?.error as { code: string; message: string; details?: { runId?: string } };
      assert.equal(error.code, 'ACTIVE_RUN_EXISTS');
      assert.equal(error.message, 'This task already has a run in progress.');
      assert.equal(error.details?.runId, 'run-already-going');
   });

   test("a quick action whose agent has left the workspace is a 409, not a 500", async () => {
      const action = await createQuickAction(sql, world.workspaceId, world.ownerId, quickActionCreateSchema.parse({ name: 'Gone', targetAgentId: world.agentId, prompt: 'Gone agent' }));
      const refused = await call('POST', `/api/v1/issues/${world.issueId}/quick-actions/${action.id}/run`, 'owner');
      assert.equal(refused.status, 409);
      assert.equal((refused.body?.error as { code: string }).code, 'CONFLICT');
   });
