...
export function planMounts(options: PlanOptions): Mount[] {
   ...
   route.post('/generate', idempotent(options.idempotency), async (context) => {
      const { value: body } = await decodeBody<{
         ...
         autoGate: 'boolean',
         // Also the dialog's: a project the AI workflow leads starts its plan
         // as soon as it is ready, here on the server.
         autoStart: 'boolean',
      });
...
async function generate(
   ...
   userId: string,
   /**
    * Start the plan here once it is ready. Asked by a project the AI workflow
    * leads: the browser used to do this from an effect, so a closed tab, a
    * crashed page or a plan opened in another tab left the plan sitting at a
    * proposal nobody would press. Like the answers path, the server sees it
    * through; the browser's own attempt then finds it already started.
    */
   autoStart = false
): Promise<void> {
   ...
   try {
      ...
      // Blocked on questions, or invalid: a person has to look, exactly as
      // when they would press Start Plan.
      ready = generated.validation.status === 'valid';
   } catch (error) {
   ...
   // Outside the try above for the same reason as after answers: a plan that
   // generated and failed to start is a compile failure, not a planning one.
   ...
   try {
      ...
      options.logger.error('plan auto-start failed', { planId: record.id, error: message });
      return;
   }
