...
import { autoDispatch, type StageGate } from '../runs/auto-dispatch.ts';
...
const KINDS = new Set(['plan', 'issueStart', 'integrationAction', 'workProposal', 'escalation']);
const MAX_TITLE = 200;
...
export interface ApprovalOptions {
   ...
   /**
    * Starts a task a decision released to an agent — an accepted proposal, an
    * answered escalation — after the decision commits. Absent where runs
    * cannot execute: the task is then released and waits to be started.
    */
   dispatch?: Pick<RunRepository, 'admit'>;
   stages?: StageGate;
   /** A released task that could not be started. The decision stands either way. */
   onDispatchError?: (issueId: string, error: unknown) => void;
}
...
export function approvalMounts(options: ApprovalOptions): Mount[] {
   ...
   for (const [path, decision] of [
      ...
      route.post(path, idempotent(options.idempotency), async (context) => {
         ...
         try {
            ...
            if (resolved.releasedIssueId) {
               await dispatchReleased(resolved.releasedIssueId, resolved.workspaceId, context.get('user').id);
            }
   ...
   /** The same auto-dispatch a person's assignment gets. Never undoes the decision. */
   async function dispatchReleased(issueId: string, workspaceId: string, userId: string): Promise<void> {
