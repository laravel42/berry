...
import { ActiveRunExists, type RunRepository } from '../runs/repository.ts';
...
export function issueTrackingRoutes(options: IssueTrackingOptions): Hono<{ Variables: AuthVariables }> {
   ...
   route.post('/:issueRef/quick-actions/:actionId/run', async (context) => {
      ...
      const { runId } = await runQuickAction(sql, options.enqueue, {
         ...
         // The queue's refusals, in the same words `POST /issues/{ref}/runs`
         // uses, so a busy task is a 409 a client can act on, not a 500.
         if (error instanceof ActiveRunExists) {
            throw new ApiError(409, 'ACTIVE_RUN_EXISTS', 'This task already has a run in progress.', {
               runId: error.runId,
            });
         }
         if (error instanceof EnqueueRejected && error.code === 'AGENT_NOT_IN_WORKSPACE') {
            throw new ApiError(409, 'CONFLICT', "This quick action's agent is no longer in this workspace.");
         }
         return rethrowWork('Quick action')(error);
      });
