...
describe(
   ...
   () => {
      ...
      test('an explicit en locale is stored and survives an unrelated settings patch', async () => {
         const patched = await call('/api/v1/me/settings', {
            method: 'PATCH',
            body: JSON.stringify({ locale: 'en' }),
         });
         assert.equal(patched.status, 200);
         assert.equal(((await patched.json()) as { locale: string }).locale, 'en');
         ...
         assert.equal(user.settings.locale, 'en');
      });
      ...
      test('a locale Berry ships no catalogue for is refused on /locale and not stored', async () => {
         ...
         assert.equal(((await after.json()) as { locale: string }).locale, 'en');
      });
      ...
      test("one user's locale never reaches another user's settings", async () => {
         ...
         const refused = await callAs(otherToken, '/api/v1/me/settings', {
            method: 'PATCH',
            body: JSON.stringify({ locale: 'ko' }),
         });
         assert.equal(refused.status, 422);
         const mine = await call('/api/v1/me/settings');
         assert.equal(((await mine.json()) as { locale: string }).locale, 'en');
      });
   }
);
