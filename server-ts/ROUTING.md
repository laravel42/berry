...
- `/api/v1/issues/{ref}/preview-environment` — a task's pull request, running. `GET` (read access) reports where it stands, its plan, log and address, and keeps it from being reaped while someone has it open; `POST` (write access, because it runs the repository's code) starts it or returns the one already running that commit, `{"force":true}` rebuilds; `DELETE` stops it. `src/previews/`: `source.ts` takes the branch head through the GitHub App; `plan.ts` reads the repository's `.berry/preview.json` or detects apps and services; `environments.ts` runs them in containers (no capabilities, bounded memory/CPU/processes, allowlisted service images, a private network, apps published on loopback only).
- Preview hosts — a request whose `Host` is `p-<id>-<app>.<BERRY_PREVIEW_DOMAIN>` never reaches the route registry: `src/previews/proxy.ts`, wrapped around the app's `fetch` in `src/index.ts`, passes it to that app's loopback port with Berry's own cookies removed. The id is 20 random hex characters that exist only while the environment does. A separate origin per app is what lets a framework app's root-absolute URLs, cookies and storage work unmodified, which a path under Berry's origin cannot.
