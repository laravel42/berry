   ...
   return (
      ...
      >
         <div className="flex items-center gap-1.5">
            <Checkbox
               checked={selected}
               className="opacity-0 data-[state=checked]:opacity-100 group-data-[selected=true]:opacity-100 mr-1 border-input data-[state=checked]:border-violet-500 data-[state=checked]:bg-violet-500 data-[state=checked]:text-white"
            />
...
export function FilterValueOptionController<TData>({
   ...
   // Remount via key={property} when the field changes. Empty deps keep
   // initialSelected stable for the selected/unselected groups; the effect
   // below only fills in options that arrive after prefetch.
   ...
   const optionCount = column.getOptions().length;

   useEffect(() => {
      if (optionCount === 0) return;
      setOptions((prev) => {
         if (prev.length === optionCount) return prev;
         const counts = column.getFacetedUniqueValues();
         return column.getOptions().map((o) => ({
            ...o,
            selected: filter?.values.includes(o.value),
   ...
   }, [column, filter?.values, optionCount]);
                  ...
                  >
                     ...
                     <TabsContent value="single" className="flex flex-col gap-4 mt-4">
                        ...
                        <div className="flex items-center gap-2">
                           <span className="font-medium">{t('value', locale)}</span>
                           <DebouncedInput
                              id="single"
                              type="number"
                     ...
                     <TabsContent value="range" className="flex flex-col gap-4 mt-4">
                        ...
                        <div className="grid grid-cols-2 gap-4">
                           <div className="flex items-center gap-2">
                              <span className="font-medium">{t('min', locale)}</span>
                              <DebouncedInput
                                 type="number"
                                 value={values[0]}
                           ...
                           <div className="flex items-center gap-2">
                              <span className="font-medium">{t('max', locale)}</span>
                              <DebouncedInput
                                 type="number"
                                 value={values[1]}
