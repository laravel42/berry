    ...
    >
      <PopoverTrigger asChild>
        <Button
          ...
          variant="outline"
          className={cn(
            'border-muted-foreground/15 shadow-none',
            iconOnly && 'px-2',
          )}
      ...
      <PopoverContent
        ...
        side="bottom"
        collisionPadding={12}
        className="w-fit overflow-visible p-0 origin-(--radix-popover-content-transform-origin)"
...
export function FilterableColumn<TData, TType extends ColumnDataType, TVal>({
  ...
  const openNested = useCallback(() => {
    // Prefetch is fire-and-forget; options resolve on the next tick while the
    // panel mounts with a stable column id so the list does not jump away.
    void prefetch()
