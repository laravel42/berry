...
export function ActivityCommentComposer({
   ...
   const canSend = Boolean(draft.trim()) && !sending && uploading === 0;
   const sendLabel = sending ? t('sending') : t('send');

   return (
      <div className={cn('flex flex-col', className)}>
         <div className="flex flex-col gap-1.5 rounded-lg border border-border bg-muted/15 px-2.5 py-1.5">
            <div className="flex items-end gap-2">
               <Button
                  variant="ghost"
                  size="icon"
                  className="mb-0.5 size-8 flex-none"
                  aria-label={t('attach')}
               ...
               <textarea
                  ...
                  className="max-h-40 min-h-8 min-w-0 flex-1 resize-none overflow-y-auto bg-transparent py-1.5 leading-5 text-foreground outline-none placeholder:text-muted-foreground"
               />
               <Button
                  size="icon"
            ...
            {slash ? (
               <ul className="z-20 max-h-48 w-72 overflow-auto rounded-md border border-border bg-popover p-1 shadow-md">
...
/** Every agent id named in the text. */
