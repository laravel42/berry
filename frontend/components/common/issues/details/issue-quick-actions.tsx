 ...
 * things that can happen are genuinely different and a toast that says
 * "started" for all of them is a lie: the run may have started, it may have
 * been refused (a task with a run already in progress takes no second one),
 * or the agent may simply have replied in the thread.
 */
      ...
      void runQuickAction(issueRef, action.id)
         .then(
            ...
            (cause: unknown): Outcome => {
               // The server's refusals, read back as what they mean.
               if (cause instanceof BerryApiError) {
                  if (cause.code === 'ACTIVE_RUN_EXISTS') {
                     return { kind: 'blocked', reason: t('blockedActiveRun') };
                  }
                  if (cause.status === 503)
                     return { kind: 'blocked', reason: t('blockedNoRuntime') };
                  if (cause.status === 409 || cause.status === 403) {
                     return { kind: 'blocked', reason: cause.message };
                  }
               }
               return { kind: 'failed' };
            }
