...
/**
 * Roles an invitation may carry. The server refuses `owner` outright, and
 * refuses `admin` unless the inviter is an owner (see `inviteRolesFor`).
 */
const INVITE_ROLES = ['admin', 'member', 'viewer'] as const;
type InviteRole = (typeof INVITE_ROLES)[number];

/** The invitation roles the server lets someone holding `role` grant. */
function inviteRolesFor(role: string | undefined): readonly InviteRole[] {
   return INVITE_ROLES.filter((candidate) => candidate !== 'admin' || role === 'owner');
}
...
export default function Members() {
   ...
   const sessionRole = useSessionStore((state) => state.workspace?.role);
   ...
   // Only an owner may invite an admin. The membership list is the freshest
   // word on this user's role; the session's workspace role covers the moment
   // before it loads. A choice the user may no longer grant (their role changed
   // while the dialog kept its last value) falls back to member.
   const inviteRoles = inviteRolesFor(me?.role ?? sessionRole);
   const chosenInviteRole: InviteRole = inviteRoles.includes(inviteRole) ? inviteRole : 'member';
         ...
         <Dialog open={inviting} onOpenChange={(open) => !sending && setInviting(open)}>
            <DialogContent>
               ...
               <div className="flex flex-col gap-3">
                  ...
                  />
                  <Select
                     value={chosenInviteRole}
                     onValueChange={(next) => setInviteRole(next as InviteRole)}
                  >
