...
function RepositoriesDirectory() {
   ...
   /**
    * GitHub reachable through the signed-in person's own token rather than an
    * App installation — the state `/github/repositories` already accepts.
    * Distinct from `installed`: the import picker and the granted list below
    * both work on this credential too, and gating them on the App alone would
    * contradict a connection that already works.
    */
   const [signInConnected, setSignInConnected] = useState(false);
   /**
   ...
   const load = useCallback(async () => {
      ...
      try {
         const [repositories, state, appState, grantState, providers] = await Promise.all([
            ...
            // Its own failure too: a page that cannot ask "is sign-in usable"
            // still has the App-based state to show.
            listProviders(workspaceId).catch(() => []),
         ]);
         ...
         setSignInConnected(
            providers.find((provider) => provider.id === 'github')?.source === 'sign-in'
         );
         setAccess(appState);
      ...
      <SettingsShell title={t('title')} description={t('lead')}>
         ...
         {loaded && !error && (
            <section className="flex flex-col gap-2">
               ...
               {/* Nothing granted through the App yet. Which sentence depends
                   on why, and every one of them names the next step —
                   including the one only an operator can take. A working
                   sign-in changes what "next" means: installing the App is an
                   upgrade here, not a requirement, because Import already
                   works on the sign-in token. */}
               {(granted?.repositories.length ?? 0) === 0 && (
                  <div className="rounded-md border border-border/60 bg-muted/20 px-3 py-2.5">
                     ...
                     {canManage && !access?.installReason && !granted?.installPending && (
                        <Button
                           size="xs"
                           variant={signInConnected ? 'secondary' : 'default'}
                           className="mt-2"
                           disabled={installing}
                           onClick={() => {
                              setInstalling(true);
         ...
         {canManage && (
            <div className="flex flex-wrap gap-2">
               ...
               <Button
                  size="xs"
                  variant="secondary"
                  disabled={(!installed && !signInConnected) || !workspaceId}
                  title={installed || signInConnected ? undefined : t('importNeedsGitHub')}
